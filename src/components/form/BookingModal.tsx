...
export type BookingCategory = "wellness" | "coach";
...
export interface CoachBookingItem extends BookingItemBase {
  category: "coach";
  sport: string;
  academy: string;
}

export type BookingItem = WellnessBookingItem | CoachBookingItem;
...
export function BookingModal({ open, item, onClose, onSuccess }: BookingModalProps) {
  ...
  const handlePay = useCallback(async () => {
    ...
    const orderPath = item.category === "coach" ? "/api/payments/coach/create-order" : "/api/payments/wellness/create-order";
    const verifyPath = item.category === "coach" ? "/api/payments/coach/verify" : "/api/payments/wellness/verify";
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-xl font-semibold">
                  Confirm {isCoach ? "Coach" : "Wellness"} Booking
                </DialogTitle>
                <p className="text-sm text-gray-200">
                  Checkout securely with Razorpay to complete your booking.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white px-6 py-5 space-y-5">
          <div className="rounded-3xl border border-gray-100 bg-slate-50 p-5">
            <div className="flex flex-wrap gap-2 items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">{isCoach ? "Coach" : "Provider"}</p>
              ...
              {isCoach ? (
                <>
                  <div className="rounded-2xl bg-white p-4 border border-gray-100">
                    <p className="text-xs uppercase tracking-[0.16em] text-gray-500">Academy</p>
                    <p className="mt-2 text-sm text-[#0B1C2D]">{item.academy}</p>
                  </div>
                  <div className="rounded-2xl bg-white p-4 border border-gray-100">
                    <p className="text-xs uppercase tracking-[0.16em] text-gray-500">Sport</p>
                    <p className="mt-2 text-sm text-[#0B1C2D]">{item.sport}</p>
                  </div>
                </>
              ) : (
                <div className="rounded-2xl bg-white p-4 border border-gray-100">
                  <p className="text-xs uppercase tracking-[0.16em] text-gray-500">Type</p>
                  <p className="mt-2 text-sm text-[#0B1C2D]">{item.type}</p>
                </div>
              )}
              <div className="rounded-2xl bg-white p-4 border border-gray-100">
